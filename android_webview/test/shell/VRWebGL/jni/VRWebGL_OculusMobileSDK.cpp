...
static void OnRemoteConnected( ovrApp* app, ovrDeviceID const deviceID )
{
...
static void UpdateRemoteDevice(ovrApp* app, const double predictedDisplayTime)
{
  ...
  if ( RemoteDeviceID != ovrDeviceIdType_Invalid )
  {
    ...
    vrapi_SetRemoteEmulation( app->Ovr, false );
  ...
  for ( bool destroyed = false; destroyed == false; )
  {
...
#endif
    {
      ...
      const ovrHeadModelParms headModelParms = vrapi_DefaultHeadModelParms();
        ...
        ovrApp::EyeParameters.rightDegrees = vrapi_GetSystemPropertyFloat(&java, VRAPI_SYS_PROP_SUGGESTED_EYE_FOV_DEGREES_X) / 2;
        ovrApp::EyeParameters.leftDegrees = vrapi_GetSystemPropertyFloat(&java, VRAPI_SYS_PROP_SUGGESTED_EYE_FOV_DEGREES_X) / 2;
        ovrApp::EyeParameters.upDegrees = vrapi_GetSystemPropertyFloat(&java, VRAPI_SYS_PROP_SUGGESTED_EYE_FOV_DEGREES_Y) / 2;
        ovrApp::EyeParameters.downDegrees = vrapi_GetSystemPropertyFloat(&java, VRAPI_SYS_PROP_SUGGESTED_EYE_FOV_DEGREES_Y) / 2;
    ...
    EnumerateInputDevices(&appState);
    UpdateRemoteDevice(&appState, predictedDisplayTime);
...
{
    pthread_mutex_lock( &ovrApp::HeadTrackingInfoMutex );
    eyeParameters.rightDegrees = ovrApp::EyeParameters.rightDegrees;
    eyeParameters.leftDegrees = ovrApp::EyeParameters.leftDegrees;
    eyeParameters.upDegrees = ovrApp::EyeParameters.upDegrees;
    eyeParameters.downDegrees = ovrApp::EyeParameters.downDegrees;
